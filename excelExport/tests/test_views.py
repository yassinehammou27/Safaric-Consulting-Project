...
class TestViews(TestCase):
    ...
    def test_ergebnisse_GET(self):
        self.client.login(username = 'testuser', password = 'secret')
        response = self.client.get(self.ergebnisse_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'auswertung/ergebnisse.html')

    def test_ergebnisse_GET_ohne_login(self):
        response = self.client.get(self.ergebnisse_url)
        self.assertEqual(response.status_code, 200)
